...
Todos los cases se renderizan con `cases/case-v3.html` (el routing en `js/app.js` manda todo a `case-v3`; las plantillas legacy `case-v2`/`case.html` fueron eliminadas). El campo `"template": "v3"` de `data/content.js` queda como marcador que lee `case-check`. El contenido es bilingüe (el mismo objeto existe en `en` y `es`, alineado por índice; el renderer usa `t(en, es)`).

**Para crear o migrar un case v3 de punta a punta** (estructura del objeto, todos los `type` con sus campos derivados del renderer, validación sin navegador y checklist): **`docs/case-v3-guide.md`** — empezá por ahí. El flujo completo input→deploy (el funnel, con qué es script y qué es opcionalmente IA): **`docs/case-pipeline.md`**. Validador: `node scripts/case-check.js <slug> --strict` (el pre-commit hook corre la versión no-strict en cada commit).
...
El index se adapta a quién mira, **sin ocultar nada** (regla: ordenar, no ocultar; sin filtros manuales). Un perfil define: `featured` (el case que va **primero en la grilla**; ya no existe sección Featured aparte, retirada 2026-07-06), `order` (resto de la grilla), `proofs` (pruebas del hero) y `label` bilingüe (sufijo "Selected Work · For payment platforms"). El **prev/next** del pie de los cases también se deriva del orden activo en runtime (los cases ya no tienen campo `nav` en `content.js`; el orden default es la posición en el array). Se activa por `?focus=<perfil>` en el index, o por el `&focus=` de un link trackeado de `go.html` (viaja por sessionStorage). **Los links trackeados NO se generan en este repo**: los genera **cbuilder** (la app externa de CVs del usuario) consumiendo el contrato **`link-spec.json`** de la raíz, que `seo-build.js` genera desde `BASE` + `data/profiles.js` (el hook bloquea drift). Cambio de dominio o perfil nuevo → regenerar → cbuilder se actualiza solo. **Privacidad**: `refToProfile` queda vacío a propósito y el spec no lleva empresas: mapearlas acá las publica (los refs de tracking son códigos opacos por esa razón). Sin perfil activo, el index no cambia en nada. `case-check` valida `profiles.js`. **Descripción completa del feature y del contrato para consumidores: `docs/personalization.md`.**
