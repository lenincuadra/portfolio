...
1. **Comprar `lenincuadra.com`** (verificado libre al 2026-07-05; ~USD 11/año en registrar at-cost). Checklist de compra: (1) cuenta en **Cloudflare Registrar** (precio de costo, WHOIS privacy gratis; requiere usar sus nameservers, que son gratis y excelentes) o **Porkbun** como alternativa simple; activar **2FA** y usar un mail que vayas a controlar siempre (quien controla la cuenta controla el dominio). (2) Comprar con **auto-renew activado** y confirmar el mail de verificación de ICANN (si no se clickea en 15 días, suspenden el dominio). (3) DNS: registros `A` de la raíz a `185.199.108.153/.109/.110/.111` + `CNAME www → lenincuadra.github.io`. (4) GitHub: repo portfolio → Settings → Pages → Custom domain `lenincuadra.com` → esperar el check DNS → activar **Enforce HTTPS** (certificado gratis automático). (5) Verificar el dominio también en Settings → Pages → *Verified domains* de la cuenta (previene takeovers). (6) Avisarme: los pasos del sitio (BASE de `seo-build`, canonical/OG/JSON-LD, `DESTINATIONS` de `go.html`, links cortos `/r/`) están en `docs/seo.md` §Reglas 3 + item (c) abajo, y los hago yo. Ojo: el sitio pasa de `…github.io/portfolio/` a la **raíz** `lenincuadra.com/` (los links viejos redirigen con 301). **Tracking al migrar**: los links viejos a `github.io` no se rompen (GitHub redirige con 301 al dominio nuevo preservando path y query, así que `go.html?ref=` sigue funcionando), pero conviene (a) actualizar los destinos de los **bit.ly del CV** al dominio nuevo para evitar el doble salto, y (b) re-testear el tracking end-to-end después del cambio (visitar `go.html?ref=test` → verificar EmailJS 200 y el dashboard, como en el registro privado). **(c) Links cortos con el dominio** (definido 2026-07-05, implementar junto con la migración): ruta `/r/<código>` servida por el truco del `404.html` de GitHub Pages (path desconocido → 404.html parsea `location.pathname`, trackea como `go.html` y redirige) → los links del CV pasan de `...go.html?ref=0705q4P&focus=payments` a **`lenincuadra.com/r/0705q4Pp`** (última letra = perfil: p/a/c). `new-link.js` imprimirá ambos formatos.
