...
**✅ Aplicado (2026-07-05):** `fintech-ecosystem` migrado a v3 (h3s declarativos, prosa, 2 videos + wireframes + research que el v2 no mostraba; pasa `case-check --strict`); las 3 `meta.description` genéricas reescritas vendiendo resultado (EN+ES); **Featured Work resucitado** (fix del bug de `hidden` en `app.js` + `ecommerce-conversion` destacado con 3 video-screens) — *evolucionado el 2026-07-06: el Featured ahora es **exclusivo de los links personalizados** (con `?focus`/ref de perfil); sin perfil, la grilla muestra los 6 cases con igual prioridad. Razón: con los perfiles vivos, el tráfico de valor llega por links personalizados y el spotlight genérico le quitaba jerarquía al resto del trabajo*; em-dashes del copy en 0 (los "70" eran en su mayoría paths de assets, el linter ahora solo mira copy); **performance**: media referenciada 19.7→10.2 MB (mockup de telecom 4.3 MB→112 KB webp; 3 videos de fintech re-encodeados a webm, el thumbnail de la card de fintech en el index pasó de 2.25 MB a 0.32 MB).

**Queda decidir** (no urgente): sección About visible — el objeto `home.about` de `content.js` no se renderiza en ningún lado (`#about` apunta al hero, que ya cubre la promesa AI); o se agrega una sección About real (los párrafos son buenos y un hiring manager los buscaría), o se borra el objeto muerto + `about.css`. **Ídem `#capabilities`** (descubierta 2026-07-05): otra sección con `hidden` permanente en `index.html` (estilos en `featured.css` §CAPABILITIES); decidir mostrarla o borrar markup + CSS.

### 🧑 Acciones tuyas (checklist de búsqueda — nadie más puede hacerlas)

1. **✅ Dominio `lenincuadra.com` comprado y migrado (2026-07-06)**: DNS only en Cloudflare, HTTPS forzado, sitio en la raíz del dominio, redirects 301 desde github.io verificados. Código migrado: BASE, canonical/OG/JSON-LD, `go.html`, og-thumbnail re-renderizado, sitemap/llms/link-spec regenerados, y **links cortos `/r/<código><P|L>[letra-perfil]`** vivos vía router en `404.html`. Tracking end-to-end **re-testeado y confirmado** (2026-07-06: link corto `/r/` en producción → mail de EmailJS recibido). **bit.ly deprecado** (2026-07-06): no se usa más en nada; los links de CVs/DMs son los cortos `/r/<código>` directos. **Search Console dado de alta con sitemap enviado (2026-07-06)**. La migración de dominio quedó 100% cerrada, sin pendientes. **Tracking al migrar**: los links viejos a `github.io` no se rompen (GitHub redirige con 301 al dominio nuevo preservando path y query, así que `go.html?ref=` sigue funcionando), pero conviene (a) actualizar los destinos de los **bit.ly del CV** al dominio nuevo para evitar el doble salto, y (b) re-testear el tracking end-to-end después del cambio (visitar `go.html?ref=test` → verificar EmailJS 200 y el dashboard, como en el registro privado). **(c) Links cortos con el dominio** (definido 2026-07-05, implementar junto con la migración): ruta `/r/<código>` servida por el truco del `404.html` de GitHub Pages (path desconocido → 404.html parsea `location.pathname`, trackea como `go.html` y redirige) → los links del CV pasan de `...go.html?ref=0705q4P&focus=payments` a **`lenincuadra.com/r/0705q4Pp`** (última letra = perfil: p/a/c). `new-link.js` imprimirá ambos formatos.
...
- **✅ Hecha (2026-07-06) — Revisión UX criterio Natoli + revisión por roles** (recruiter/HM/par/engineer/founder). Hallazgos accionables, **pendientes de aprobación** (son cambios de diseño/copy, decide el usuario):
  1. **Jerarquía del hero invertida**: el elemento visualmente dominante es el nombre, pero lo que vende (la frase de valor y sobre todo los **Highlights con las métricas**) está en el texto más chico del bloque, casi letra legal. Las pruebas duras deben ganar peso visual (el recruiter escanea 10-30s).
  2. **Resolver el About de una vez**: el nav promete "About" pero apunta al hero; los HM buscan bio/trayectoria y no existe. O sección About corta (los párrafos + skills ya están escritos en `content.js`) o quitar el item del nav.
