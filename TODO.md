...
5. **Un link trackeado por aplicación**, generado con **`node scripts/new-link.js "<empresa>" --focus <perfil>`**: crea el código opaco, arma las URLs (portfolio + LinkedIn) y registra la empresa en el registro privado. El `--focus` personaliza la primera pantalla para el que entra (solo revela industria, nunca la empresa). Para links sin tracking (mails/DMs): `?focus=payments|ai|conversion` directo sobre el index. Saber quién visitó define a quién hacerle follow-up.
