...
**✅ Aplicado (2026-07-05):** `fintech-ecosystem` migrado a v3 (h3s declarativos, prosa, 2 videos + wireframes + research que el v2 no mostraba; pasa `case-check --strict`); las 3 `meta.description` genéricas reescritas vendiendo resultado (EN+ES); **Featured Work: resucitado → evolucionado → retirado (2026-07-06 final)**: la sección costaba ~2 pantallas de scroll en mobile antes de mostrar amplitud; la personalización vive ahora en el **orden de la grilla + sufijo del label** ("Selected Work · For payment platforms") con las mismas señales y menos fricción. Además el **prev/next de los cases se deriva del orden activo** en runtime (campo `nav` eliminado de `content.js`) y se agregó el link central "All work"; em-dashes del copy en 0 (los "70" eran en su mayoría paths de assets, el linter ahora solo mira copy); **performance**: media referenciada 19.7→10.2 MB (mockup de telecom 4.3 MB→112 KB webp; 3 videos de fintech re-encodeados a webm, el thumbnail de la card de fintech en el index pasó de 2.25 MB a 0.32 MB).

**✅ Resuelto (2026-07-06)**: sección **About real agregada** después de Selected Work (label + heading + párrafos + skills desde `content.js`, bilingüe; el nav `#about` ancla ahí y el spy ya no marca About en el hero). `#capabilities` eliminada (2026-07-06). `about.css` reescrito sin los estilos muertos (bio-block/venn-diagram).
